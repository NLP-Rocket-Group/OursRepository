...
class DataPreProcessAndWord2VecTrain:
...
# ----以下为汉语中文语料处理----
    ...
    def sentence_decode(sentence):
        try:
            ...
            sentence = sentence.decode("GB18030")
    ...
    def string_process(string):
        ...
        stay = []
        new_string = DataPreProcessAndWord2VecTrain.cut_sentence(string) #这个分句设计可能还是有问题的
    ...
    def han_path_laod(input_csv_path, output_txt_path):
        ...
        for count,article in enumerate(data.content):
            process_article = DataPreProcessAndWord2VecTrain.string_process(str(article))
...
if __name__ == '__main__':
    ...
    # DataPreProcessAndWord2VecTrain.han_path_laod(han_path,txt_path)

    #处理 wiki中文语料，1.7G大小，1327个文件

    # all_wiki_path = DataPreProcessAndWord2VecTrain.read_all_wiki_path()
    # DataPreProcessAndWord2VecTrain.strip_wiki_source(all_wiki_path)

    #切分七千万条数据左右

    # DataPreProcessAndWord2VecTrain.get_cut_lines("wiki_stripped.txt")

    #wiki语料训练
    # DataPreProcessAndWord2VecTrain.train_word2vec("cut_lines.txt")

    #中文新闻预料训练
    # DataPreProcessAndWord2VecTrain.update_word2vec(data_path=txt_path,model_path=model_path)
